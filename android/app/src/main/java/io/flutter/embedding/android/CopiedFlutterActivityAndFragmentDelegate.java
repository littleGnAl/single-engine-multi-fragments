 ...
 * behavior.</strong>
 */
/* package */ class CopiedFlutterActivityAndFragmentDelegate implements ExclusiveAppComponent<Activity> {
    private static final String TAG = "CopiedFlutterActivityAndFragmentDelegate";
    private static final String FRAMEWORK_RESTORATION_BUNDLE_KEY = "framework";
    private static final String PLUGINS_RESTORATION_BUNDLE_KEY = "plugins";

    // The FlutterActivity or FlutterFragment that is delegating most of its calls
    // to this CopiedFlutterActivityAndFragmentDelegate.
    ...
    CopiedFlutterActivityAndFragmentDelegate(@NonNull Host host) {
        this.host = host;
    }

    /**
     * Disconnects this {@code CopiedFlutterActivityAndFragmentDelegate} from its host {@code Activity} or
     * {@code Fragment}.
     *
     * <p>No further method invocations may occur on this {@code CopiedFlutterActivityAndFragmentDelegate}
    ...
    public @NonNull Activity getAppComponent() {
        ...
        if (activity == null) {
            throw new AssertionError(
                    "CopiedFlutterActivityAndFragmentDelegate's getAppComponent should only "
    ...
    void detachFromActivityIfNeeded() {
        if (!isDetached() && host.shouldAttachEngineToActivity()) {
            // Notify plugins that they are no longer attached to an Activity.
            Log.v(TAG, "Detaching FlutterEngine from the Activity that owns this Fragment.");
            if (host.getActivity().isChangingConfigurations()) {
                flutterEngine.getActivityControlSurface().detachFromActivityForConfigChanges();
    ...
    private void ensureAlive() {
        if (host == null) {
            throw new IllegalStateException(
                    "Cannot execute method on a destroyed CopiedFlutterActivityAndFragmentDelegate.");
        }
    }
...
//        }
     ...
     * CopiedFlutterActivityAndFragmentDelegate}.
     */
